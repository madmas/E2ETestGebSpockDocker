...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('server', 'Creates configuration for the grunt-contrib-connect grunt task and runs it', function () {
        ...
        var user = options.proxy.user;

        if (user) {
            grunt.log.write('Loading user: %s ...', user.green);
            var https = {
                ca  : grunt.file.read(path.join('certs', 'mobi-root-test-ca.pem')),
                key : grunt.file.read(path.join('certs', 'user', user, 'mobi-user-test-key.pem')),
                cert: grunt.file.read(path.join('certs', 'user', user, 'mobi-user-test-cert.pem'))
            };
        }
        ...
        if (targets) {
            targets = normalizeTargets(targets, https);
